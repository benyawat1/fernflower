...
public class MethodProcessorRunnable implements Runnable {
  ...
  public static RootStatement codeToJava(StructClass cl, StructMethod mt, MethodDescriptor md, VarProcessor varProc) throws IOException {
    ...
    if (DecompilerContext.getOption(IFernflowerPreferences.ENSURE_SYNCHRONIZED_MONITOR)) {
      // ecial case: search for 'synchronized' ranges w/o monitorexit instruction (as generated by Kotlin and Scala)
      DeadCodeHelper.extendSynchronizedRangeToMonitorExit(graph);
    }

    if (DecompilerContext.getOption(IFernflowerPreferences.NO_EXCEPTIONS_RETURN)) {
      // ecial case: single return instruction outside of a protected range
    ...
    if (ExceptionDeobfuscator.hasObfuscatedExceptions(graph)) {
      ...
      if (!ExceptionDeobfuscator.handleMultipleEntryExceptionRanges(graph)) {
        DecompilerContext.getLogger().writeMessage("Found multiple entry exception ranges which could not be litted", IFernflowerLogger.Severity.WARN);
      }
    ...
    do {
      StackVarrocessor.simplifyStackVars(root, mt, cl);
